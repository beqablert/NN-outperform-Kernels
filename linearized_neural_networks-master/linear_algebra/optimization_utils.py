	...
	def __init__(self, loss, preds, params, labels, reg_param, num_obs):        		
		...
		with tf.compat.v1.variable_scope('v'):
			for variable in params:
				v_variable = tf.compat.v1.get_variable(variable.op.name, shape=variable.shape,\
					initializer=tf.compat.v1.ones_initializer(tf.float32), trainable=False, dtype=tf.float32)
