...
class MultiLayerNetwork(object):
	def __init__(self, data, labels, params):
		...
		self._train_op = None
		self._lr = tf.compat.v1.placeholder(dtype=tf.float32, name='learning_rate', shape=[])
		if form_train_op:
			self._form_train_op()
	...
	def _fcLayer(self, x, num_hiddens, name, include_bias, trainable, std):
		...
		with tf.compat.v1.variable_scope(name):			
			w = tf.compat.v1.get_variable(initializer=tf.constant_initializer(W0), name="W", shape=[d, N], dtype=tf.float32, trainable=trainable)
			if include_bias:
				b = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.01), name="bias",\
		...
		output = tf.compat.v1.nn.dropout(output, keep_prob=1.0 - self._drop_rate)
	...
	def _compute_accuracy(self, yhat, labels):
		if self._expandedFinalLayer:
			adjusted_label = tf.compat.v1.to_int32(labels[:, 0])
			class_pred = tf.argmax(yhat, axis=1, output_type=tf.int32)
			zero_one_loss_vec = tf.compat.v1.to_float(tf.equal(class_pred, adjusted_label))
			assert len(zero_one_loss_vec.shape) == 1
		else:			
			# Note this is only valid for two-class classification			
			adjusted_label = tf.compat.v1.to_int32(tf.sign(labels - self._mean))
			class_pred = tf.sign(yhat - self._mean)
			class_pred = tf.compat.v1.to_int32(class_pred)
			zero_one_loss_vec = tf.compat.v1.to_float(tf.equal(class_pred, adjusted_label))		
...
class TwoLayerReluNT(MultiLayerNetwork):
	...
	def _model(self, data, labels):		
		...
		at = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.0),\
			name='layer2', shape=[N, num_classes], dtype=tf.float32)	
		c = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.0),\
			name='layer2_bias', shape=[1, num_classes], dtype=tf.float32)				
		z = tf.matmul(data, w)
		q = tf.nn.relu(z)
		RF = tf.matmul(q, at) + c # BS x num_classes
			
		G = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.0),\
...
class RF(MultiLayerNetwork):
	def __init__(self, data, labels, params):
		...
		print(self._N)
		self._d = int(data.shape[1])
		self._tph = tf.compat.v1.placeholder(tf.float32, (self._d, self._N))
	...
	def _model(self, data, labels):		
		...
		#w = tf.constant(W0, name='fixed_w', shape=[d, N], dtype=tf.float32)
		#del W0, norms
		w = tf.compat.v1.get_variable(name='fixed_w', shape=[d, N], trainable=False, dtype=tf.float32)
		self._rf_assign = tf.compat.v1.assign(w, self._tph)
		...
		p_vec = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.0), name='Hvec', shape=[N, 1], trainable=False, dtype=tf.float32)
		self._assign_op = tf.compat.v1.assign(p_vec, self._pten)
...
class FullyConnected(MultiLayerNetwork):
	...
	def _model(self, data, labels):
		...
		a = tf.compat.v1.get_variable(initializer=tf.random_normal_initializer(stddev=1.0), name='Softmax_layer', shape=[N, num_classes], dtype=tf.float32)		
		c = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.0), name='Softmax_bias', shape=[1, num_classes], dtype=tf.float32)
	...
	def _form_train_op(self):
		...
		if self._optimizer == 'mom':
			opt = tf.compat.v1.train.MomentumOptimizer(learning_rate=self._lr, momentum=0.9)
		elif self._optimizer == 'sgd':
			opt = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=self._lr)
		elif self._optimizer == 'adam':
			opt = tf.compat.v1.train.AdamOptimizer(learning_rate=self._lr)
		else:
			raise Exception('Optimizer was not recognized!')
		self._grads_vars = opt.compute_gradients(self._loss, self._variables)		
...
class Myrtle(MultiLayerNetwork):
	...
	def _model(self, data, labels):
		...
		print(x.shape)
		N = np.int(x.shape[1])
		a = tf.compat.v1.get_variable(initializer=tf.random_normal_initializer(stddev=1.0 / np.sqrt(N + 0.0)), name='Softmax_layer', shape=[N, self._num_classes], dtype=tf.float32)
		c = tf.compat.v1.get_variable(initializer=tf.constant_initializer(0.0), name='Softmax_bias', shape=[1, self._num_classes], dtype=tf.float32)
