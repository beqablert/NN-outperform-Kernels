...
flags.DEFINE_integer('num_units', 5, 'The number of hidden units')
...
class Experiment(object):
	def __init__(self):
		...
		with graph.as_default():
			...
			self._batch_size = tf.compat.v1.placeholder(tf.int64, shape=[])
	...
	def _save_results(self, save_dict, exp_flag):
		...
		print('Regularization Loss is %f'%(save_dict['reg_loss']))		
		print('Train loss is %f'%(save_dict['train_loss']))            		
		print('Test loss is %f'%(save_dict['test_loss']))	
	...
	def _data_pipeline(self, model):
		if self._model_name in ['Myrtle']:
			self._features_placeholder = tf.compat.v1.placeholder(self._X.dtype, (None, self._X.shape[1], self._X.shape[2], self._X.shape[3]))
		else:
			self._features_placeholder = tf.compat.v1.placeholder(self._X.dtype, (None, self._X.shape[1]))
		self._labels_placeholder = tf.compat.v1.placeholder(self._Y.dtype, (None, 1))
		if self._model_name in ['rf', '2layerNTK']:
			dataset = tf.compat.v1.data.Dataset.from_tensor_slices((self._features_placeholder, self._labels_placeholder)).repeat(1).batch(self._batch_size, drop_remainder=True)
		else:
			dataset = tf.compat.v1.data.Dataset.from_tensor_slices((self._features_placeholder, self._labels_placeholder)).shuffle(128).repeat(1).batch(self._batch_size, drop_remainder=True)
	...
	def _record_stats(self, sess, mode, model, iterator, batch_size):
		...
		while not end_of_data:
			try:
				...
				end_of_data = True  
		return (np.mean(loss_list), np.mean(acc_list))   #shows the mean train/accuracy of all the batches 
